...
q = 0
qtrs = ['20{}Q{}'.format(str(i).zfill(2), j) for i in range(9,17) for j in range(1,5)]
random.shuffle(qtrs)
data = None
min_index = 0
max_index = 0
...
gen = generator(batch_size = batch_size, steps = steps, lookforward = lookforward)
val_gen = generator(batch_size = batch_size, steps = steps, lookforward = lookforward)
test_gen = generator(batch_size = batch_size, steps = steps, lookforward = lookforward)
model.compile(optimizer = RMSprop(), loss = 'binary_crossentropy', metrics = ['acc'])
model.fit(gen, steps_per_epoch = epoch_steps, epochs = epochs, validation_data = val_gen, validation_steps = val_steps)

model.evaluate(test_gen, steps = testing_steps)
model.save('model2009.h5')
s3.Bucket('loan-performance-data').upload_file('model2009.h5', 'saved_models/model2009.h5')
