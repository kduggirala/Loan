...
def generator(batch_size, steps, lookforward, starting_from = 0, ending_at = 1, testing = False):
    ...
    global max_index
    k = 0
    samples = np.zeros((batch_size, (lookforward//steps) - 1, 5))
    ...
    while True:
        ...
        while j < max_index:
            ...
            if target == 0:
                if count < 10:
                    samples[k] = slice[:(lookforward//steps) - 1, :5]
                    targets[k] = target
                    k += 1
                    count += 1
                j += 1
...
model = Sequential()
model.add(layers.LSTM(32, dropout = dropout, recurrent_dropout = dropout, input_shape = ((lookforward//steps - 1), 5)))
...
s3.Bucket('loan-performance-data').upload_file('model5.h5', 'saved_models/model5.h5')
