...
def make_model(qtrs_train, batch_size, lookforward, steps, epoch_steps, val_steps, testing_steps, partition, num_layers, neurons, dropout, optimizer, epochs):
    ...
    s3.Bucket('loan-performance-data').upload_file(model_name, 'saved_models/rolling_models2/'+model_name)
...
make_model(qtrs_train, 128, 12, 1, 5000, 1000, 2500, 5, 2, 32, 0.2, optimizers.RMSprop(), 10)
...
for layer_count in layers_counts:
    for batch_size in batch_sizes:
        for neurons in neuronss:
            for dropout in dropouts:
                make_model(qtrs_train, batch_size, 12, 1, 5000, 1000, 2500, 5, layer_count, neurons, dropout, optimizers.RMSprop(), 10)
